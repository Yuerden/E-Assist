...
async function formulateSummary(tone, length, emailContent) {
    try {
        const response = await openai.chat.completions.create({
            model: 'gpt-3.5-turbo',
            messages: [{
                role: 'user',
                content: `"Given the following email, generate a ${tone} response that addresses all the questions or points raised:

                Email Content:"${emailContent}"
                
                Please ensure the response is ${length} and [Action-Oriented/Question-Answering/Information-Requesting/Feedback-Giving], reflecting a [Urgency] sense of urgency.
                ,`,
            }],
            temperature: 0.5,
            max_tokens: 2048,
